...
| **SCHEMA GAP** | 0 | **3** | Pydantic models missing fields that both platform + domain YAMLs use |
...
|---|------|---------|-------------------|
| H3 | `blueprints/agent-blueprint.md` | Strategy type `SUMMARIZATION` | Enum has `SUMMARY` only. Domain YAML also uses wrong value. |
...
## Block 3 — Schema Gaps (code bugs, not doc bugs)

These are cases where BOTH platform example YAMLs AND domain repo YAMLs use fields that don't exist in the Pydantic models. They work today because Pydantic silently ignores unknown fields (no `extra="forbid"`).

| # | Schema | Missing Fields | Used By |
|---|--------|----------------|---------|
| SG1 | `WorkflowState` | `agent_ref`, `prompt`, `input_mapping`, `error`, `cause`, `comment`, `heartbeat_seconds` | Platform YAMLs + domain repo |
| SG2 | `WorkflowBlueprint` | `memory_branching` (with `enabled`, `merge_strategy`, `branch_namespace`) | Domain repo `weekly_analysis.yaml` |
| SG3 | `StrategyBlueprint` | `evaluation`, `risk_controls` | Domain repo strategy YAMLs |

**These are NOT documentation issues — they are missing Pydantic fields that should be added to the schemas.**
...
### Phase 3 — Schema Gaps (code bugs to fix separately)
11. Add `agent_ref`, `prompt`, `input_mapping` etc. to `WorkflowState` Pydantic model
12. Add `memory_branching` to `WorkflowBlueprint`
13. Add `evaluation`, `risk_controls` to `StrategyBlueprint` (or explicitly document as unsupported)
