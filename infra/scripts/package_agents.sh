...
find "$DIST_DIR" -name '*.dist-info' -type d -exec rm -rf {} + 2>/dev/null || true

echo "==> Creating ZIP..."
cd "$DIST_DIR"
zip -r "$INFRA_DIR/lambda/agents/agents.zip" . -x '*.pyc' '__pycache__/*' '*.dist-info/*'
