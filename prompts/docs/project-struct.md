...
**`resolver.py`** — Query layer. `parse_prompt_ref()` handles three reference formats: plain name (`my_agent`), underscore-v (`my_agent_v1.2`), and at-sign (`my_agent@1.2.0`). `PromptResolver` combines registry + storage to resolve references to actual text, respecting mode gates — production agents can never see drafts, dev/simulation can fall back to latest draft when no stable exists.
...
| `fixtures/sample_prompts/` | Two versions of a my_agent prompt (v1.0.0 basic, v1.2.0 with percentage thresholds and JSON schema) |
